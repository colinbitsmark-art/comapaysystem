    ...
    <>
      {uploads.map((upload, index) => (
        <div
          ...
          onDrop={(e) => {
            onDrop(e, index, "receipt");
            setActiveUploadType(null);
          }}
          onDragOver={(e) => {
            onDragOver(e, "receipt");
            setActiveUploadType("receipt");
          }}
          onDragLeave={(e) => {
            onDragLeave(e, "receipt");
            setActiveUploadType(null);
          }}
        ...
        >
          {(!upload.image && !upload.amount && !upload.accountId) && (
            <button
              ...
              onClick={(e) => {
                ...
                if (newUploads.length === 0 && setShowReceiptUpload) {
          ...
          <input
            ...
            ref={(el) => {
              if (el) {
                const key: string = `receipt-${uploadKey}-${index}`;
                fileInputRefs.current[key] = el;
              }
            }}
            key={`receipt-file-${uploadKey}-${index}`}
            className="hidden"
            id={`receipt-file-input-${uploadKey}-${index}`}
            onChange={(e) => onFileChange(e, index, "receipt")}
          />
          <label
            htmlFor={`receipt-file-input-${uploadKey}-${index}`}
          ...
          >
            {upload.image ? (
              ...
              <div className="w-full h-full flex flex-col items-center justify-center border-2 border-dashed border-slate-300 rounded-lg hover:border-blue-400 transition-colors">
                ...
                <p className="mt-2 text-xs text-slate-600">{t("orders.clickOrDragToUpload")}</p>
              </div>
            )}
          </label>
          <input
